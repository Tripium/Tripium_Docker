...
class ReviewController extends Controller
{
    ...
    private function isDiscussionAllowed($post): string
    {
    ...
    {
        if (!is_singular(General::getPostTypeKeys(false)) && !is_author()) {
            return false;
        }
        ...
        if ($post instanceof WP_Post) {
            $aData["reviews"] = [
                ...
                'isReviewed' => !ReviewModel::isEnabledReview($post->post_type) || ReviewModel::isUserReviewed($post->ID) ? 'yes' : 'no',
                'mode'       => ReviewModel::getReviewMode($post->post_type),
                'myReview'   => empty($aMyReviews) ? [] : $aMyReviews
            ];
        }

        wp_localize_script('wilcity-empty', 'WIL_REVIEW_CONFIGURATION', $aData);
    }
