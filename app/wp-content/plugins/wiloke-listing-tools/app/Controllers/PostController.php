...
//        add_action('wiloke/submitted-listing', [$this, 'clearScheduleAfterSubmittingListing']);
    ...
    public function validateExpiredListing($post)
    {
        ...
        if ($isExpiredListing && !Session::getPaymentPlanID(false) && ($post->post_author == get_current_user_id()
            ...
            echo(
            sprintf(
                __('<div class="wil-listing-expired-notice">The listing has been expired on %s. Click <a style="color: red; font-weight: bold" href="%s">here</a> to republish the listing</div>',
    ...
    public function updatePostAfterPaymentCompleted($aInfo)
    {
        ...
        $this->clearAllSchedules($aPaymentMetaInfo["postID"]);
...
//        file_put_contents(plugin_dir_path(__FILE__) . "payment-completed.json", json_encode($aInfo));
    }
    ...
    public function deleteListing()
    {
        ...
        wp_delete_post($_POST['postID'],
            apply_filters('wilcity/app/Controllers/PostController/deleteListing/isFocus', true));
    ...
    protected function setScheduleExpiration($postID, $expirationTimestamp)
    {
        ...
        $postID = absint($postID);
        $expirationTimestamp = is_numeric($expirationTimestamp) ? $expirationTimestamp :
            strtotime($expirationTimestamp);
        $now = current_time('timestamp');

        $expirationTimeReformat = $this->getAlmostExpiredDate($expirationTimestamp);

        $beforeOneWeek = $this->getAlmostExpiredDate($expirationTimestamp);
        if (Time::compareTwoTimes($expirationTimeReformat, $now, 7)) {
            wp_schedule_single_event($beforeOneWeek, $this->almostExpiredKey, [$postID]);
        }

        $beforeFiveDays = $this->getAlmostExpiredDate($expirationTimestamp, 5);
        if (Time::compareTwoTimes($expirationTimeReformat, $now, 5)) {
            wp_schedule_single_event($beforeFiveDays, $this->almostExpiredKey, [$postID]);
        }

        $beforeTwoDays = $this->getAlmostExpiredDate($expirationTimestamp, 2);
        if (Time::compareTwoTimes($expirationTimeReformat, $now, 2)) {
            wp_schedule_single_event($beforeTwoDays, $this->almostExpiredKey, [$postID]);
        }
        ...
        if (!empty($belongsTo)) {
            ...
            if (isset($aPlanSettings['regular_period']) && !empty($aPlanSettings['regular_period'])) {
                if ($expiryTimestamp < current_time('timestamp')) {
                    ...
                    $expiryTimestamp = strtotime('+ ' . $aPlanSettings['regular_period'] . ' days', $expiryTimestamp);
                }


                SetSettings::setPostMeta($postId, 'post_expiry', $expiryTimestamp);
//                file_put_contents(plugin_dir_path(__FILE__) . "newdata.json", json_encode([
//                    'expiryTimestamp' => $expiryTimestamp,
//                    'current_time' => current_time('timestamp'),
//                    'postId' => $postId,
//                    'belongsTo' => $belongsTo,
//                    'aPlanSettings' => $aPlanSettings
//                ]));
            }
        }
