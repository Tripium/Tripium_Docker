...
final class PlanRelationshipModel
{
    ...
    public static function getLastRelationshipIdOfObject($objectID, $planID, $authorID)
    {
        global $wpdb;
        $table = $wpdb->prefix . AlterTablePlanRelationships::$tblName;
        $planID = GetWilokeSubmission::getOriginalPlanId($planID);
        $id = $wpdb->get_var(
            $wpdb->prepare(
    ...
    public static function upgradeToNewPaymentID($newPaymentID, $oldPaymentID, $oldPlanID, $newPlanID, $userID = null)
    {
        ...
        return $wpdb->update(
            $table,
            [
                'planID'    => GetWilokeSubmission::getOriginalPlanId($newPlanID),
                'paymentID' => $newPaymentID
            ],
            [
                'userID'    => $userID,
                'planID'    => GetWilokeSubmission::getOriginalPlanId($oldPlanID),
    ...
    public static function upgradeListingToNewPlan($aInfo, $userID = null)
    {
        ...
        return $wpdb->update(
            $table,
            [
                'planID'    => GetWilokeSubmission::getOriginalPlanId($aInfo['planID']),
                'paymentID' => $aInfo['paymentID']
            ],
            [
                'userID'    => $userID,
                'planID'    => GetWilokeSubmission::getOriginalPlanId($aInfo['oldPlanID']),
    ...
    public static function update($id, $aInfo)
    {
        ...
        foreach ($aInfo as $key => $val) {
            if ($key == "planID") {
                $aValues[$key] = GetWilokeSubmission::getOriginalPlanId($val);
            } else {
                $aValues[$key] = $val;
            }
            $aPrepares[] = '%d';
    ...
    public static function setPlanRelationship($aInfo)
    {
        ...
        $aInfo['planID'] = GetWilokeSubmission::getOriginalPlanId($aInfo['planID']);
    ...
    private static function addLeeDays() {
        return apply_filters("wiloke-listing-tools/app/Models/PlanRelationshipModel/addLeeDays/minutes", 0);
    }
...
$planRelationshipTbl.paymentID=%d AND $planRelationshipTbl.userID=%d",
    ...
    public static function getUsedRecurringPlan(RemainingItems $that): int
    {
        global $wpdb;
        $planRelationshipTbl = $wpdb->prefix . AlterTablePlanRelationships::$tblName;
        $timestampToDate = Time::toAtomUTC($that->getNextBillingDateGMT() - self::addLeeDays());
