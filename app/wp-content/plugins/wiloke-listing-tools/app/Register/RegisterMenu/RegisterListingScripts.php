...
class RegisterListingScripts
{
    ...
    public function getPostTypes(\WP_REST_Request $request)
    {
        ...
        if ($request->get_param('action') == 'listing_post_types') {
            ...
            $aPostTypes = General::getPostTypes();
        }
    ...
    public function reviewSettings()
    {
        ...
        if (isset($aData['details'])) {
            ...
            SetSettings::setOptions(General::getReviewKey('details', $_POST['postType']),
                self::unSlashDeep($aData['details']), true);
        } else {
    ...
    public function saveUsedAddListingSections()
    {
        ...
        if (!isset($_POST['results']) || empty($_POST['results'])) {
            ...
            SetSettings::setOptions(General::getUsedSectionKey($_POST['postType']), $this->cleanOptions($aValues),
                true);
        }
    ...
    public function saveSearchFields()
    {
        ...
        SetSettings::setOptions(General::getSearchFieldsKey($_POST['postType']), self::unSlashDeep($_POST['data']),
            true);
    ...
    public static function getHighlightBoxes($postType)
    {
        ...
        self::$aHighlightBoxes = GetSettings::getOptions(
            General::getSingleListingSettingKey('highlightBoxes', $postType),
            false,
            true
        );
    ...
    private function setDefaultBodyListingCard($postType)
    {
        ...
        SetSettings::setOptions(General::getSingleListingSettingKey('card', $postType), self::unSlashDeep($aDefault),
            true);
        ...
        SetSettings::setOptions(General::getSingleListingSettingKey('footer_card', $postType),
            self::unSlashDeep($aFooterSettings), true);
    ...
    public function saveSchemaMarkupSettings()
    {
        $this->validateAjaxPermission();
        if (empty($_POST["data"])) {
            SetSettings::setOptions(General::getSchemaMarkupKey($_POST['postType']), [], false);
        } else if (is_array($_POST['data'])) {
            SetSettings::setOptions(General::getSchemaMarkupKey($_POST['postType']), json_encode($_POST['data']),
                false);
        } else if (isJson($_POST["data"])) {
            SetSettings::setOptions(General::getSchemaMarkupKey($_POST['postType']), $_POST["data"], false);
        }

        SetSettings::setOptions(
            General::getSchemaMarkupSavedAtKey($_POST['postType']),
    ...
    protected function getUsedBodyListingCard()
    {
        $postType = General::detectPostTypeSubmission();
        $postType = $this->getPostType($postType);
        $aUsedListingCardBody = GetSettings::getOptions(General::getSingleListingSettingKey('card', $postType), false,
            true);
