...
class GetSettings
{
    ...
    public static function isEnableCustomLoginPage(): bool
    {
        if (!class_exists('\WilokeThemeOptions')) {
            return false;
        }

        if (defined("WILCITY_FOCUS_DISABLE_CUSTOM_LOGIN_PAGE") && WILCITY_FOCUS_DISABLE_CUSTOM_LOGIN_PAGE) {
            return false;
        }


        $status = WilokeThemeOptions::isEnable('toggle_custom_login_page');
        if ($status) {
            $customLoginPageID = WilokeThemeOptions::getOptionDetail('custom_login_page');
            if (!empty($customLoginPageID)) {

                if (get_post_status($customLoginPageID) == 'publish') {
                    return true;
                }
            }
        }
