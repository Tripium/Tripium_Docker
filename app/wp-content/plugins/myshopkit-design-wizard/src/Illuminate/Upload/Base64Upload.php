...
class Base64Upload extends AUpload {
	...
	protected function _processUpload( array $aFile ): array {
		...
		if ( ! $uploadFile ) {
			return [
				'msg'    => sprintf( esc_html__( 'Failed to upload %s', 'myshopkit-design-wizard' ),
					$this->_fileName ),
				'status' => 'error'
			];
		}
	...
	public function processUpload(): array {
		...
		if ( ! $this->_fileMimeType ) {
			return MessageFactory::factory( 'normal' )->error( esc_html__( 'Invalid file type', 'myshopkit-design-wizard' ), 415 );
		}
