...
class ImageURLUpload extends AUpload {
	...
	private function _getFileInfo(): AUpload {
		...
		if ( ! $this->_fileMimeType ) {
			throw new \Exception( esc_html__( 'Invalid Image URL', 'myshopkit-design-wizard' ) );
		}
		...
		try {
			...
			if ( $attachmentId = $this->_getAttachmentIdByOriginalImg() ) {
				return MessageFactory::factory( 'normal' )
				                     ->success(
					                     esc_html__( 'All files have been uploaded successfully.', 'myhopkit-design-wizard' ),
					                     [
						                     'item' => [
							                     'id'     => $attachmentId,
							                     'url'    => wp_get_attachment_image_url( $attachmentId, 'full' ),
			...
			return MessageFactory::factory( 'normal' )
			                     ->success(
				                     esc_html__( 'All files have been uploaded successfully.', 'myshopkit-design-wizard' ),
				                     [
					                     'item' => $aResponse
				                     ]
			                     );
