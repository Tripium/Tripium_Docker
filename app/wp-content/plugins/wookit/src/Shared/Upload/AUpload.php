...
namespace WooKit\Shared\Upload;
...
abstract class AUpload
{
    ...
    public function setImageSource(string $source): AUpload
    {
        if (!in_array($source, $this->_aValidSources)) {
            throw new RuntimeException(esc_html__('Invalid source', 'wookit'));
        }
    ...
    public function setUpdateAttachmentId(int $_updateAttachmentId): AUpload
    {
        if ($this->_isAttachmentAuthor($_updateAttachmentId)) {
            ...
            throw new Exception(esc_html__('You are not author of this attachment', 'wookit'));
        }
    ...
    public function setType(string $type): AUpload
    {
        if (in_array($type, $this->aValidTypes)) {
            ...
            throw new RuntimeException(sprintf(esc_html__('Invalid image type %s', 'wookit'), $type));
        }
    ...
    protected function _validateFileSize($fileSize): bool
    {
        if ($fileSize > wp_max_upload_size()) {
            throw new Exception(sprintf(esc_html__('The file is large than expected %sM', 'wookit'),
    ...
    protected function _validateExtension($fileMine): bool
    {
        if (!in_array($fileMine, $this->aExtensions)) {
            throw new Exception(sprintf(esc_html__('The file type %s is not allowed', 'wookit'),
                $fileMine));
        }
    ...
    protected function _proceedUpdateOrInsertAttachment(): array
    {
        if (empty($this->_fileMimeType)) {
            throw new Exception(esc_html__('The file mime type is required', 'wookit'));
        }

        if (empty($this->_fileName)) {
            throw new Exception(esc_html__('The file name type is required', 'wookit'));
        }

        if (empty($this->_extension)) {
            throw new Exception(esc_html__('The extension is required', 'wookit'));
        }

        if (!in_array($this->_imgSource, $this->_aValidSources)) {
            throw new Exception(esc_html__('The image source is not allowed', 'wookit'));
        }
        ...
        if (!empty($this->_updateAttachmentId)) {
            ...
            if (!$status) {
                throw new Exception(esc_html__('We could not update the data', 'wookit'));
            }
        ...
        return [
            ...
            'msg'    => sprintf(
                esc_html__('The file %s has been uploaded successfully', 'wookit'),
                $this->_fileName
            ),
            'status' => 'success'
        ];
