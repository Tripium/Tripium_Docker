...
class WPUpload extends AUpload
{
    ...
    private function _prepareImage(): AUpload
    {
        if (
            ...
            throw new RuntimeException(esc_html__('Invalid parameter', 'wookit'));
        }
        ...
        switch ($this->singularFile['error']) {
            ...
            case UPLOAD_ERR_NO_FILE:
                throw new RuntimeException(esc_html__('No file sent.', 'wookit'));
            case UPLOAD_ERR_INI_SIZE:
            case UPLOAD_ERR_FORM_SIZE:
                throw new RuntimeException(esc_html__('Exceeded filesize limit.', 'wookit'));
            default:
                throw new RuntimeException(esc_html__('Unknown errors.', 'wookit'));
        }
    ...
    protected function _processUpload(array $aFile): array
    {
        ...
        if (!isset($aFile['tmp_name']) || empty($aFile['tmp_name'])) {
            return [
                'status' => 'error',
                'msg'    => esc_html__('Sorry, We could not download this file.', 'wookit')
            ];
        }
        ...
        if (!$status) {
            return [
                'status' => 'error',
                'msg'    => esc_html__('Sorry, We could not copy this file.', 'wookit')
            ];
        }
